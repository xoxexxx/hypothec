...
$message .= "Стоимость недвижимости: {$data['results']['cost']} <br>";
$message .= "Первоначальный платеж: {$data['results']['firstPayment']} <br>";
$message .= "Срок в годах: {$data['results']['term']} <br>";
$message .= "<hr>";

// Результаты расчета
$message .= "<h2>Результаты расчета</h2>";
$message .= "Процентная ставка: {$data['results']['rate']} <br>";
$message .= "Сумма кредита: {$data['results']['sum']} <br>";
$message .= "Ежемесячный платеж: {$data['results']['payment']} <br>";
$message .= "Переплата: {$data['results']['overpay']} <br>";
